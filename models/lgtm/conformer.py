...
from layers import GLU, PositionEncoding, ResidualConnection
from utils.tensor import Permute
...
class ConformerAttentionModule(nn.Module):
    ...
    def forward(self, src: Tensor, src_key_padding_mask: Union[Tensor, None] = None) -> Tensor:
        x: Tensor = src
        x = x + self._token_self_attn_block(x, src_key_padding_mask)
        x = x + self._emb_self_attn_block(x)
        return x

    def _token_self_attn_block(self, x: Tensor, mask: Union[Tensor, None] = None) -> Tensor:
...
class ConformerBlock(nn.Module):
    ...
    def forward(
        self,
        src: Tensor,
        src_key_padding_mask: Union[Tensor, None] = None,
    ) -> Tensor:
        x: Tensor = src
